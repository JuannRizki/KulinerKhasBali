  ...
  <div class="container mx-auto flex items-center justify-between px-4 py-3">
    ...
    <div class="flex items-center gap-4">
      ...
      <form action="{{ route('user.menu.index') }}" method="GET" class="hidden md:flex items-center gap-2">
        ...
        <button type="submit" class="bg-white text-green-600 rounded-full px-4 py-2 font-semibold text-sm">
          Search
        </button>
      </form>
      ...
      <a href="{{ route('cart.index') }}"
        class="relative inline-flex items-center justify-center text-white hover:text-green-200">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round"
            d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2 9m13-9l2 9m-5-4h-4" />
        </svg>
        <span
          class="absolute top-0 right-0 inline-block w-4 h-4 text-xs bg-red-600 text-white rounded-full text-center leading-4">
          {{ $cartCount ?? 0 }}
        </span>
      </a>
      ...
      <div class="relative group">
        ...
        <ul
          class="absolute right-0 mt-2 w-40 bg-white rounded-md shadow-lg hidden group-hover:block z-10">
          <li>
      ...
      @else
      <!-- Tombol Login kalau belum auth -->
      <a href="{{ route('login') }}"
        class="text-white font-semibold hover:text-green-200">Login</a>
      @endauth
    </div>
  </div>
</nav>
