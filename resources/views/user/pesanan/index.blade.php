...
<div class="container mx-auto py-6">
  <h2 class="text-2xl font-bold mb-6">Daftar Pesanan Saya</h2>
    ...
    <div class="grid grid-cols-1 gap-4">
      @foreach($pesanans as $pesanan)
        <div class="relative border p-4 rounded shadow bg-white">
          ...
          {{-- Tombol silang untuk batalkan --}}
          @if(in_array($pesanan->status, ['unpaid', 'waiting_verification', 'pending']))
            <form action="{{ route('pesanan.batal', $pesanan->id) }}" method="POST" onsubmit="return confirm('Yakin ingin membatalkan pesanan ini?');" class="absolute top-2 right-2">
              @csrf
              <button class="text-red-500 hover:text-red-700 text-xl leading-none" title="Batalkan Pesanan">&times;</button>
            </form>
          @endif
